...
export default async function MeetingTypeBookingPage({
  ...
  const hostSlug = resolvedParams.slug;
  const meetingTypeSlug = resolvedParams.meetingType;
  ...
  // ============================================================================
  const quotaStatus = await getHostBookingQuotaStatus(resolvedParams.slug);
  ...
  const [{ data: meetingTypeData }, { data: bookings }] = await Promise.all([
    sanityFetch({
      ...
      params: { 
        hostSlug, 
        meetingTypeSlug // This matches the query: slug.current == $meetingTypeSlug
      },
    }),
    sanityFetch({
    ...
    <main className="min-h-screen bg-linear-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        ...
        <BookingCalendar
          ...
          // Use the slug from the URL params directly
